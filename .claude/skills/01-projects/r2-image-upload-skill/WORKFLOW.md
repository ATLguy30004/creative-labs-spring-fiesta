...
After setup, commit `.r2-upload.json` and `.r2-manifest.json` to git so your whole team shares upload state.
...
│   │   └── hero.webp
│   └── logo.svg
├── .r2-upload.json   ← config (committed to git)
└── .r2-manifest.json ← upload state (committed to git)
```

Images in `_images/` are a **local staging area only** — R2 is the source of truth. The manifest tracks what's been uploaded so your team doesn't re-upload unchanged files.
...
| Auth keeps expiring | Create a persistent API token — see `references/cloudflare-token-permissions.md` |
