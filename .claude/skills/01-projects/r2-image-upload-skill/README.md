...
2. In Claude Code, run:
   ```
   /install-skill r2-image-upload-skill-v1.0.0.zip
   ```
...
> **Advanced:** For persistent auth or CI/CD use, you can set a `CLOUDFLARE_API_TOKEN` env var instead. See `references/cloudflare-token-permissions.md` for required permissions.
...
3. **Manifest:** `.r2-manifest.json` tracks what's uploaded — commit it to git so your team shares state
