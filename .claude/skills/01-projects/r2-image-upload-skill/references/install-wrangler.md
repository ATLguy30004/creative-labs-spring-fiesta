...
**Want persistent auth?** — `wrangler login` can expire. For long-term use, create an API token with the permissions documented in `references/cloudflare-token-permissions.md` and set it as `CLOUDFLARE_API_TOKEN` in the shell profile.
